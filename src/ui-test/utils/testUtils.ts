...
export const RESOURCES_COMMAND: string = path.resolve('src', 'ui-test', 'resources', 'camel_route_command');
...
export const SPECIFIC_WORKSPACE: string = path.resolve('src', 'ui-test', 'resources', 'create-camel-project-workspace');
...
export async function initXMLFileWithJBang(driver: WebDriver, filename: string): Promise<void> {
	...
	await driver.wait(async function () {
		return (await new EditorView().getOpenEditorTitles()).find(t => t === filename.concat('.camel.xml'));
	}, 220000);
}
