...
describe('Code navigation', function () {
     ...
     * @param listOfAvailableSymbols List of expected symbols with line number of occurence.
     */
    async function gotoSymbolsUsingQuickpickCommand(listOfAvailableSymbols: any, title: string): Promise<void> {
        for (const quickpick of quickpicks) {
            await selectSymbolFromProposals(listOfAvailableSymbols.at(quickpick.getIndex()).at(0));
